...
The provider status feed is an optional server-side integration. Its version-1 JSON object contains `version: 1`, an ISO-8601 UTC `generatedAt`, and a `providers` array; each row contains an exact canonical `slug`, `status` (`operational`, `degraded`, `outage`, or `unknown`), and ISO-8601 UTC `checkedAt`, with optional bounded `description` and absolute HTTP(S) `url`. The WebUI fetch has a 3-second wall-clock bound across connection, headers, and body reads, plus a 256 KiB body limit; it sends fixed non-sensitive headers, accepts only fresh rows, and fails soft by hiding public context. Public rows annotate the local quota card and never replace local account state.
