...
def test_raw_audio_active_recording_uses_dedicated_i18n_key():
    assert "voice_recording_active" in I18N_SOURCE
    assert I18N_SOURCE.count("voice_recording_active") == LOCALE_COUNT
...
def test_sidebar_lineage_tooltip_suffixes_are_localized():
    assert I18N_SOURCE.count("session_lineage_toggle_hint") == LOCALE_COUNT
    assert I18N_SOURCE.count("session_lineage_static_hint") == LOCALE_COUNT
...
def test_sidebar_child_tooltip_suffix_is_localized():
    assert I18N_SOURCE.count("session_child_toggle_hint") == LOCALE_COUNT
...
def test_read_only_session_title_hint_is_localized():
    assert I18N_SOURCE.count("session_readonly_title_hint") == LOCALE_COUNT
