// i18n core: eager metadata, English fallback, and locale readiness.
// Non-English translation data is registered by static/locales/<code>.js.
...
const _I18N_CORE_URL = (() => {
  if (typeof document === 'undefined' || !document.currentScript) return 'static/i18n-core.js';
  return document.currentScript.src || 'static/i18n-core.js';
})();
const _I18N_VERSION_QUERY = (() => {
  try { return new URL(_I18N_CORE_URL, document.baseURI || location.href).search; }
  catch (_) { return ''; }
})();

function _localeAssetUrl(code) {
  const relative = `locales/${code}.js${_I18N_VERSION_QUERY}`;
  if (typeof URL !== 'function') return `static/${relative}`;
  return new URL(relative, new URL(_I18N_CORE_URL, document.baseURI || location.href)).href;
}

function getLocaleMetadata(code) {
  const resolved = resolveLocale(code);
  return resolved ? LOCALE_REGISTRY[resolved] : null;
}
...
function setLocale(lang) {
  ...
  const bundle = LOCALES[resolved] || LOCALES.en;
  _locale = bundle;
  const active = bundle._lang || 'en';
  _activeLocale = active;
  try { localStorage.setItem('hermes-lang', active); } catch (_) {}
  if (typeof document !== 'undefined' && document.documentElement) {
    document.documentElement.lang = bundle._speech || active;
  }
  return active;
}

function getActiveLocale() {
  return _activeLocale || (_locale && _locale._lang) || 'en';
}

async function activateLocale(lang) {
  const requested = resolveLocale(lang) || 'en';
